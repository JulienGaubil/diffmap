...
class ResizeFlow:
    def __init__(self, new_size: int | tuple[int,int]):
    ...
    def __call__(self, flow: Float[Tensor, "xy=2 height width"]) -> Float[Tensor, "xy=2 height_new width_new"]:
        flow_resized = FT.resize(flow, [self.new_size_H, self.new_size_W], interpolation=transforms.InterpolationMode.BILINEAR)
