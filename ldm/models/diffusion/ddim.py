...
class DDIMSamplerDiffmap(DDIMSampler):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None, dynamic_threshold=None,
                      t_start=-1) -> tuple[Sample, list[Float[Tensor, '...']]]:
        device = self.model.betas.device
        b = shape[0]
        ...
        for i, step in enumerate(iterator):
            ...
            samples = self.p_sample_ddim(samples, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      quantize_denoised=quantize_denoised, temperature=temperature,
                                      noise_dropout=noise_dropout, score_corrector=score_corrector,
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
    ...
    @torch.no_grad()
    def p_sample_ddim(self, samples: Sample, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None,
                      dynamic_threshold=None):
        ...
        if score_corrector is not None:
            assert self.model.parameterization == "eps"
            e_t = score_corrector.modify_score(self.model, e_t, x_noisy, t, c, **corrector_kwargs)
