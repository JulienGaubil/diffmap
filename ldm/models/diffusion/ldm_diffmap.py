...
class FlowMapDiffusion(LatentDiffusion): #derived from LatentInpaintDiffusion
    ...
    def get_input_flowmap(
            self,
            depths_recon_flowmap: dict[str, Float[Tensor, "batch channel height width"]],
        ...
        depths_recon = torch.stack([
            depths_recon_flowmap["depth_ctxt"].mean(1),
            depths_recon_flowmap["depth_trgt"].mean(1)
            ],
            dim=1
        )
        # Normalize the depth
    ...
    def p_losses(self, z_start, cond, flows, flows_masks, correspondence_weights, t, noise=None):
        ...
        depths_recon_flowmap = self.decode_first_stage_all(z_recon, modalities=["depth_trgt", "depth_ctxt"])
        dummy_flowmap_batch, flows, depths_recon, correspondence_weights = self.get_input_flowmap(depths_recon_flowmap, flows, flows_masks, correspondence_weights)
